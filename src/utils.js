...
const getResultsFromGoogleMaps = (string, callback) => {
  if (typeof string === "string" && typeof callback === "function") {
    let fullURL = "https://maps.googleapis.com/maps/api/geocode/json?address=" + string + "&key=WRONG_KEY";
    // console.log(fullURL);
    fetch(fullURL)
      .then(response => {
 ...
 * @param {Boolean} noNulls
 */
const xyObjectFromKeyValue = (data, propertyX, propertyY = "TotalCases", noNulls = true) => {
  ...
  data.forEach(feature => {
    ...
    if (noNulls && x !== null) {
      ...
      if (map.get(x)) {
        ...
        map.set(typeof x === "number" ? +x : x, typeof y === "number" ? +y : y);
      }
    }
  });
...
const generateDeckLayer = (name, data, renderTooltip, options) => {
  const addOptionsToObject = (opt, obj) => {
    Object.keys(opt).forEach(key => (obj[key] = opt[key]));
  };
  if (name === "hex") {
    const hexObj = {
      id: "hexagon-layer",
...
const convertRange = (
  ...
  let value =
    ((oldValue - values.oldMin) * (values.newMax - values.newMin)) / (values.oldMax - values.oldMin) + values.newMin;
...
const shortenName = (name, n = 26) => {
  ...
  const extension = name.split(".").length > 1 && name.split(".").pop().length < 10 && name.split(".").pop();
...
const isURL = str => {
  ...
  a.href = str;
  return a.host && a.host !== global.window.location.host;
};

const isMobile = function() {
  var check = false;
  if (global.window.innerWidth < 640) return true;
  (function(a) {
    if (
      ...
      check = true;
  })(navigator.userAgent || navigator.vendor || global.window.opera);
  return check;
};
...
const colorScale = (d, features, p = 0, alpha = 180) => {
  ...
  let domain = features.map(feature => {
    ...
    if (isNumber(i) && p === "Mean.Travel.Time..Seconds.") {
  ...
  const index = domain.indexOf(
    isNumber(d.properties[x]) && p === "Mean.Travel.Time..Seconds."
      ? Math.floor(d.properties[x] / 300)
      : d.properties[x]
  );
  ...
  col = col
    ...
    .replace(/ /g, "")
    .split(",")
    .map(x => +x); // deck.gl 8 int not strings
  return [...col, alpha];
};

const colorRangeNames = ["inverseDefault", "yellowblue", "greens", "oranges", "diverge", "default"];
...
const searchNominatom = (location, callback) => {
  const url = "https://nominatim.openstreetmap.org/search/" + location + "?format=json";
  fetchData(url, json => {
    ...
    <g id="ATI_logo_black_ATI_logo_black_W1024px" fill={dark ? "#ffffff" : "#000000"}>
      <g
        ...
        <path
          ...
          id="Fill-13"></path>
        <path
          d="M117.531,128.642 L143.027,128.642 L143.027,83.08 C143.027,63.192 148.647,53.667 160.371,53.667 C171.081,53.667 175.162,59.62 175.162,74.751 L175.162,128.642 L200.658,128.642 L200.658,72.031 C200.658,57.749 198.622,50.437 193.181,44.156 C187.738,37.861 178.392,33.78 169.209,33.78 C159.354,33.78 149.664,38.713 143.027,47.042 L143.027,0.793 L117.531,0.793 L117.531,128.642"
          id="Fill-12"></path>
        <polyline
          id="Fill-11"
          points="55.645 0.796 0.819 0.796 0.819 23.242 42.3 23.242 42.3 128.632 69.158 128.632 69.158 23.242 110.474 23.242 110.474 0.796 55.645 0.796"></polyline>
      </g>
      <g
        id="logo-line-2"
...
const generateDomain = (data, column) => {
  if (!data || !Array.isArray(data) || !column || !isString(column) || data.length === 0) return;

  let domainIsNumeric = true;
  let domain = data.map(feature => {
    // uber move to show isochrones
    const i = feature.properties[column];
    if (isNumber(i) && column === "Mean.Travel.Time..Seconds.") {
      return Math.floor(i / 300);
    }
    return isNumber(i) ? +i : i;
  });
