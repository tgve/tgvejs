...
const generateTooltip =(props) => {
  ...
  //no more selectedObject.cluster
  const cluster = selectedObject && selectedObject.points
  ...
  if (!type_feature && !cluster) {
    ...
    const severity_keys = column1 && getPropertyValues(
      { features: selectedObject.points.map(e => ({properties: e.properties})) },
      column1);
    const severity_by_year = column1 && column2 &&
      propertyCountByProperty(
        selectedObject.points.map(e => ({properties: e.properties})),
        column1, column2);
  ...
  const tooltip =
    <div
      ...
      <div>
        <b>Total: {cluster ? selectedObject.count :
...
const listPropsAndValues = (selectedObject, all = false, n = 6) => {
    if(!selectedObject.properties && !selectedObject.points) return null
    let DATA = []
