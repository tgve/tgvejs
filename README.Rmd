...
* `geographyURL`, if provided and both on initialization and when `reset` button is pressed, data is fetched separately using this variable and `defaultURL` variables.

* `geographyColumn` a matching column between data return from `defaultURL` and `geographyURL`. This is the joining column that will result in dynamically generating `geojson` data for eAtlas to consume.
...
See the main repo guide for more details but you can use a template ([`layik/eatlas-template`](https://github.com/layik/eatlas-template)) repo to publish your data using eAtlas. 
